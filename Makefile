...
IMAGE_TAG_BASE ?= $(REGISTRY)/ndd-core
...
IMG_CORE ?= $(REGISTRY)/nddcore:latest
IMG_RBAC ?= $(REGISTRY)/nddrbac:latest
...
manifests: controller-gen ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	$(CONTROLLER_GEN) rbac:roleName=manager-role crd paths=./apis/... webhook paths="./..." output:crd:artifacts:config=config/crd/bases
...
NDD_GEN ?= $(LOCALBIN)/ndd-gen

## Tool Versions
KUSTOMIZE_VERSION ?= v3.8.7
CONTROLLER_TOOLS_VERSION ?= v0.8.0
KUBECTL_NDD_VERSION ?= v0.2.20
NDD_GEN_VERSION ?= v0.1.1
...
.PHONY: ndd-gen
ndd-gen: $(NDD_GEN) ## Download kubectl-ndd locally if necessary.
$(NDD_GEN): $(LOCALBIN)
	GOBIN=$(LOCALBIN) go install github.com/yndd/ndd-tools/cmd/ndd-gen@$(NDD_GEN_VERSION)  ;\
